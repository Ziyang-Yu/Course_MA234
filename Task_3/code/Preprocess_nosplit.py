...
def fill_pm_data():
    ...
    dataset_Y = np.load(os.path.abspath(os.path.dirname(os.path.dirname(__file__))) + "/data/dataset_Y.npy")

    dataset_X = dataset_X.tolist()
    dataset_Y = dataset_Y.tolist()


    for i in range(3, len(dataset_X)):
        dataset_X[i].append(dataset_Y[i-1])
        dataset_X[i].append(dataset_Y[i-2])
        dataset_X[i].append(dataset_Y[i-3])
