...
for epoch in range(EPOCHS):
    ...
    if avg_vloss < best_vloss:
        ...
        if platform.system().lower() == 'windows':
            print("windows")
            model_path = file_path + '\Task_3\code\models\LSTM_model_average'
        elif platform.system().lower() == 'linux':
            print("linux")
            model_path = file_path + '/Task_3/code/models/LSTM_model_average'
...
for epoch in range(EPOCHS):
    ...
    if avg_vloss < best_vloss:
        ...
        if platform.system().lower() == 'windows':
            print("windows")
            model_path = file_path + '\Task_3\code\models\GRU_model_average'
        elif platform.system().lower() == 'linux':
            print("linux")
            model_path = file_path + '/Task_3/code/models/GRU_model_average'
        torch.save(model.state_dict(), model_path)
...
for epoch in range(EPOCHS):
    ...
    if avg_vloss < best_vloss:
        ...
        if platform.system().lower() == 'windows':
            print("windows")
            model_path = file_path + '\Task_3\code\models\Transformer_model_average'
        elif platform.system().lower() == 'linux':
            print("linux")
            model_path = file_path + '/Task_3/code/models/Transformer_model_average'
        torch.save(model.state_dict(), model_path)
...
model_lstm.load_state_dict(torch.load('models/LSTM_model_average'))
model_lstm.eval()

### gru
model_gru = gru.gru_model()
model_gru.load_state_dict(torch.load('models/GRU_model_average'))
model_gru.eval()

### transformer
model_transformer = transformer.transformer_model()
model_transformer.load_state_dict(torch.load('models/Transformer_model_average'))
